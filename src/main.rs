...
fn main() {
    // let weights: tensor::Tensor<f32, 3, 4> = Tensor::from_data([[0.0, 1.0, 0.0, 3.0],[0.0, -0.91, 0.26, -0.5],[0.0, -0.27, 0.17, 0.87]]);
    ...
    let acc: Accuracy<300, 3> = Accuracy{};
    println!("{:?}", acc.calculate(activator_res_2, Targets::onehot(test_targets())));
}